...
export default function FichaUO() {
  ...
  const [logForm, setLogForm] = useState({ porcentaje_avance: 0, fase: 'Preparacion', nota: '' })
...
observaciones: uoData.observaciones || '',
  ...
  async function guardarLog() {
    ...
    if (!error) { setShowLogForm(false); fetchAll() }
    setSavingLog(false)
  }
          ...
          <div style={{ padding:'14px 20px', borderBottom:'0.5px solid var(--border2)' }}>
            ...
            {showLogForm && (
              <div style={{ background:'var(--surface2)', border:'0.5px solid rgba(249,115,22,0.2)', borderRadius:'7px', padding:'14px', marginBottom:'10px', display:'flex', flexDirection:'column', gap:'10px' }}>
                ...
                <div>
                  <div style={{ fontFamily:'var(--mono)', fontSize:'8px', color:'var(--muted2)', marginBottom:'4px' }}>NOTA (opcional)</div>
                  <textarea rows={2} value={logForm.nota} onChange={e => setLogForm(f => ({ ...f, nota: e.target.value }))} style={{ resize:'vertical', fontSize:'11px' }} />
                </div>
                <div style={{ display:'flex', gap:'6px', justifyContent:'flex-end' }}>
                  <button onClick={() => setShowLogForm(false)} style={{ padding:'5px 12px', borderRadius:'4px', border:'0.5px solid var(--border)', background:'none', color:'var(--muted2)', fontSize:'9px' }}>CANCELAR</button>
            ...
            {historial.map((h, i) => (
              <div key={h.id} style={{ display:'grid', gridTemplateColumns:'16px 1fr', gap:'10px', paddingBottom:'10px' }}>
                ...
                <div style={{ paddingBottom:'10px', borderBottom: i < historial.length-1 ? '0.5px solid var(--border2)' : 'none' }}>
                  ...
                  <div style={{ fontFamily:'var(--mono)', fontSize:'8px', color:'var(--muted2)' }}>{h.usuario?.nombre} · {new Date(h.created_at).toLocaleDateString('es-MX')}</div>
                </div>
              </div>
            ))}
          </div>
