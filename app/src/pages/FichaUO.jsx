...
console.log('Error:', transError)
  ...
  async function guardarAsignacion() {
    ...
    if (asignacion.digitalizador_id && !uo.fecha_asignacion) {
      ...
      await supabase.from('historial_estados').insert({
        ...
        estado_anterior: uo.estado, estado_nuevo: 'Asignada',
  ...
  async function guardarLog() {
    ...
    if (noAvanza && !logForm.nota.trim()) {
      setLogError('Explica por que el avance no progreso.')
      return
    }
    if (logForm.fase === 'Carga completa' && (!logForm.comentario_entrega.trim() || logForm.comentario_entrega.trim().length < 20)) {
      setLogError('Agrega un comentario de entrega para el validador (minimo 20 caracteres).')
      return
    }
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
              ...
              <span style={{ fontFamily:'var(--mono)', fontSize:'8px', padding:'2px 8px', borderRadius:'3px',
                ...
                color: uo.prioridad==='P1' ? 'var(--orange)' : 'var(--muted2)' }}>
                {uo.prioridad}
              </span>
            </div>
          </div>
              ...
              <div style={{ background:'var(--surface2)', border:'0.5px solid rgba(249,115,22,0.2)', borderRadius:'7px', padding:'14px', marginBottom:'10px', display:'flex', flexDirection:'column', gap:'10px' }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  <div>
                    ...
                    {noAvanza && logs.length > 0 && (
                      <div style={{ fontFamily:'var(--mono)', fontSize:'8px', color:'var(--yellow)', marginTop:'4px' }}>
                        Ultimo registro: {ultimoPct}% — nota obligatoria
                      </div>
                    )}
                  </div>
                  <div>
                ...
                <div>
                  <div style={{ fontFamily:'var(--mono)', fontSize:'8px', color: noAvanza ? 'var(--orange)' : 'var(--muted2)', marginBottom:'4px' }}>
                    NOTA {noAvanza ? '* OBLIGATORIA' : '(opcional)'}
                  </div>
                  <textarea rows={2} value={logForm.nota} onChange={e => setLogForm(f => ({ ...f, nota: e.target.value }))}
                    ...
                    style={{ resize:'vertical', fontSize:'11px', borderColor: noAvanza && !logForm.nota.trim() ? 'rgba(249,115,22,0.4)' : undefined }} />
                </div>
                {logForm.fase === 'Carga completa' && (
                  <div>
                    ...
                    <textarea rows={3} value={logForm.comentario_entrega}
                      onChange={e => setLogForm(f => ({ ...f, comentario_entrega: e.target.value }))}
                      placeholder="Que debe saber el validador antes de revisar esta UO?"
                      style={{ resize:'vertical', fontSize:'11px', borderColor: logForm.comentario_entrega.trim().length < 20 ? 'rgba(249,115,22,0.4)' : undefined }} />
                ...
                <div style={{ paddingBottom:'10px', borderBottom: i < historial.length-1 ? '0.5px solid var(--border2)' : 'none' }}>
                  ...
                  {h.motivo_texto && (
                    <div style={{ display:'flex', flexDirection:'column', gap:'3px' }}>
                      {h.categoria_error && (
                        <span style={{ fontFamily:'var(--mono)', fontSize:'7px', padding:'1px 6px', borderRadius:'3px', background:'rgba(249,115,22,0.12)', color:'var(--orange)', display:'inline-block', width:'fit-content' }}>
                          {h.categoria_error}
                        </span>
                      )}
                      <div style={{ fontFamily:'var(--mono)', fontSize:'9px', color:'var(--muted2)', fontStyle:'italic' }}>
                        "{h.motivo_texto}"
                      </div>
                    </div>
                  )}
